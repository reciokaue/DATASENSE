      ...
      {form != null && !isPending ? (
        <>
          ...
          <header className="flex flex-col space-y-2 px-4 py-3 text-start sm:px-6 md:px-10">
            ...
            <Badge variant="secondary" className="!mt-4 text-sm sm:text-base">
              Enviamos suas respostas anonimamente
            </Badge>
          </header>
          <form
            onSubmit={handleSubmit(onSubmit)}
            className="flex h-full w-full flex-1 flex-col px-4 sm:px-6 md:px-10"
          >
              ...
              {form?.questions?.map((question, index) => (
                <section key={question.id} className="flex w-full flex-col">
                  <header className="mb-3 flex items-center gap-2">
                    <span className="flex size-7 items-center justify-center rounded-full bg-primary text-center text-sm text-primary-foreground">
                      {index + 1}
                    </span>
